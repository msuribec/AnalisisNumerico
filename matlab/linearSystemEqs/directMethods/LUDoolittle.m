function [xi,L,u] = LUDoolittle (A,b)
...
%LUDoolittle([Matriz A],[Vector B])
%ejm LUDoolittle([2 -1 0 3;1 0.5 3 8;0 13 -2 11;14 5 -2 3],[1;1;1;1])
text=fopen('tabla_Doolittle.txt','w');
fprintf(text,'                     FACTORIZACION LU DOOLITTLE\n\n\n');
...
if n==m
    for k=1:n
      ...
      %numero espec�fico de veces  
        ...
        u(k,k)=(A(k,k)-suma);
        texto=['U(' num2str(k) ',' num2str(k) ')' num2str(u(k,k))];
        fprintf(text,texto);
        fprintf(text,'\n');
        for i=k+1:n
            suma=0;
